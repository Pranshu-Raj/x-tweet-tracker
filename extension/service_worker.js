...
// for the app origin, these fetches are CORS-exempt. When the hosted app has a token
// set, every request carries it via the x-cockpit-token header (configured in the popup).
...
// Single fetch wrapper: attaches JSON content-type (when there's a body) and the
// access token (when configured) to every call.
async function api(base, path, opts = {}) {
  const { cockpitToken } = await chrome.storage.sync.get("cockpitToken");
  const headers = { ...(opts.headers || {}) };
  if (opts.body != null && !headers["content-type"]) headers["content-type"] = "application/json";
  if (cockpitToken) headers["x-cockpit-token"] = cockpitToken;
...
async function saveFollowers(base, followers) {
  const r = await api(base, "/api/growth", { method: "POST", body: JSON.stringify({ followers }) });
...
async function saveTweet(base, t) {
  ...
  const list = await (await api(base, "/api/drafts")).json();
  ...
  if (match) {
    ...
    const r = await api(base, "/api/drafts/" + match.id, {
      method: "PATCH",
  ...
  const created = await (
    await api(base, "/api/drafts", { method: "POST", body: JSON.stringify({ body: t.body }) })
  ).json();
  const id = created.draft && created.draft.id;
  if (!id) throw new Error("create failed");
  await api(base, "/api/drafts/" + id, {
    method: "PATCH",
...
async function saveActivity(base, tweets, replies) {
  ...
  const res = await api(base, "/api/activity", {
    method: "POST",
...
async function saveReplyCapture(base, tweet, handle) {
  const res = await api(base, "/api/reply-capture", {
    method: "POST",
...
async function saveProfile(base, profile) {
  const res = await api(base, "/api/profile", { method: "POST", body: JSON.stringify(profile) });
  if (!res.ok) throw new Error("profile " + res.status);
  return "profile captured → open Coach ✓";
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    try {
      ...
      else if (msg.type === "profile") m = await saveProfile(base, msg.profile);
